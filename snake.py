...
class Snake():
    ...
    def on_key(self, event):
        if event.key != self.opposite_direction and event.key in self.opposites:
    ...
    def move(self):
        ...
        if self.direction=="right":
            self.x = [(self.x[0]+1)%self.M] + self.x[:-1]
            self.y = [self.y[0]] + self.y[:-1]
        elif self.direction=="left":
            self.x = [(self.x[0]-1)%self.M] + self.x[:-1]
            self.y = [self.y[0]] + self.y[:-1]
        elif self.direction=="up":
            self.y = [(self.y[0]+1)%self.N] + self.y[:-1]
    ...
    def eat(self):
        if self.direction=="right":
            self.x = [(self.x[0]+1)%self.M] + self.x
            self.y = [self.y[0]] + self.y
        elif self.direction=="left":
            self.x = [(self.x[0]-1)%self.M] + self.x
            self.y = [self.y[0]] + self.y
        elif self.direction=="up":
            self.y = [(self.y[0]+1)%self.N] + self.y
    ...
    def dead(self):
        ...
        self.ax.annotate(message, xy=(self.M/2, self.N/2), fontsize=24,
...
class Food():
    def __init__(self, snake):
        ...
        self.new_pos()
    def new_pos(self):
        self.x = int(random()*self.snake.M)
        self.y = int(random()*self.snake.N)
        self.plot.set_data(self.x, self.y)
